...
We have added a layer of shoe images to AlexNet. [AlexNet](https://github.com/BVLC/caffe/tree/master/models/bvlc_alexnet) is a pre-trained convolutional neural network that has been trained on ~1,000,000 images. After getting trained on the shoe images, by adding only one layer to AlexNet, the new deep CNN can ow transform the input shoe image into a 40 dimensional feature space. It is then much easier to find similar images in the new space (using kNN for example). This method really shines, as you'll see in the next section.  
...
BruteForce returns results that are not are very satisfing. 
<p align="center">
